...
SENSOR_MAP = [ 
        {
            ...
            'label': 'environmental.{}',
            'tags': 'room=office;location=window'
        },
        {
            # Ruuvi #2
            'address': 'C1:2F:56:FB:6A:0A',
            'label': 'environmental.{}',
            'tags': 'room=bedroom;location=window'
        },
        {
            # Ruuvi #3
            'address': 'F6:8C:F2:8D:6E:A3',
            'label': 'environmental.{}',
            'tags': 'room=living;location=tv'
        },
        {
            # Mijia #1
            'address': '4C:65:A8:D4:03:E3',
            'label': 'environmental.{}',
            'tags': 'room=hallway;location=kitchen'
        },
    ]

def send_sensor_data(msg):
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.connect(CARBON_SERVER)
    s.send(msg)
    s.close()
    s = None
